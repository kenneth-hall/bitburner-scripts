...
| `src/bladeburnerdiag.js` | ~10.6 GB | **Rewritten 2026-09-07** into a read-only *ladder-readiness* probe: rank, SP bank, stamina fraction, HP, city chaos, all 12 skill levels, `getNextBlackOp()`, and `[pMin, pMax]` / action time / rank gain for **every remaining black op** — plus a marginal-value simulation of what spending the whole SP bank buys on the BI x DO success multiplier. Writes `bladeburnerprobe-<epoch>.json` (the vite filter matches the FILE, not the script). Replaces the old trial-era spot-check (rank / current action / Raid chance), whose question died with `bladeburnertrial.js`. |
