...
| Gang API (`ns.gang.*` + `ns.formulas.gang.*`): full method/type surface, the "must have joined gang" precondition, bonus-time 25× mechanic, what's computable vs measurable | [gang-api.md](gang-api.md) | **Before designing anything gang-related** — a Phase 27 draft written off the method list alone had its premise invalidated twice by facts in this file |
