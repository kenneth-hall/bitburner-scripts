...
// daemon-log.json -- the only file download.location below lets through --
// gets pulled to logs/ on a timer instead of needing someone at the dev
// terminal to press it.
function autoExportDaemonLog(): Plugin {
  return {
    name: 'auto-export-daemon-log',
    configureServer() {
      setInterval(() => {
