...
|---|---|---|---|
| **Q40-6** | Is `[0.60, 0.95]` the right proxy for the realised-yield peak, or hill-climb on `byLevel.rankSum/attempts`? | **Ship the band** (cost if wrong ~1.5% of aggregate; `byLevel` makes the hill-climb a re-analysis) | ≥200 attempts across ≥3 levels for one action. **2026-08-18** |
| **Q40-7** | Does `setActionLevel` on a running rep reset/corrupt its progress? | **Never find out** — step 5's forced restart moots it | If the restart shows as a duty-cycle cost. **2026-08-24** |
| **Q40-8** | Does the ~+4%/level payout slope (Contract-measured) hold for Operations? | **Assume yes; the controller does not depend on it** (it reads success rate, never payout) | Free from the ledger once `Investigation` settles at ≥3 levels. **2026-08-18** |
...
| **Q40-13** | Is `LEVEL_MAX_TOTAL_DROP = 12`/24 h right? | **12**, regime resets exempt. A `drop-budget-exhausted` warn outside a cohort collapse means the constant is wrong | First such warn. **Event-triggered** |
| **Q40-14** | 🆕 **Phantom failures in the denominator:** a verified start whose rep is preempted before completing counts as an attempt with 0 successes | **Accept.** Bounded by measured cadence — inter-start gaps match action times, no restart churn (1 gap < 30 s in 703), so switches align with completions; contamination is ≲1–2% and biases toward lowering, the cheap direction | If L2 shows `Tracking` (true ~100% success) settling materially below 1.0, phantom starts are the first suspect. **2026-08-18** |
